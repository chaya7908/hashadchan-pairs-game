const WAIT_BEFORE_CHECK_MATCH_INDICATION = 4000;
...
function createCard(candidate, type) {
    ...
    let propertiesHTML = '<strong>Properties:</strong>';
    for (const [key, value] of Object.entries(candidate.properties)) {
        if (value !== undefined) {
            propertiesHTML += `<div>${key}: ${value}</div>`;
        }
    }

    // Dynamically create looking for HTML
    let lookingForHTML = '<strong>Looking For:</strong>';
    for (const [key, value] of Object.entries(candidate.lookingFor)) {
        if (value !== undefined) {
            lookingForHTML += `<div>${key}: ${value}</div>`;
        }
    }

    cardBack.innerHTML = `<div>${propertiesHTML}${lookingForHTML}</div>`;
...
function propertiesMatch(properties, lookingFor) {
    return Object.keys(lookingFor).every(key => properties[key] === lookingFor[key]);
}
